...
Table~\ref{tab:objective} summarizes the objective metrics across all 500 validation pairs. DeFX improves SI-SDR by +6.30\,dB on average, reduces L1 error by 44\%, and cuts mel cepstral distortion by more than half.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lrrr@{}}
    ...
    \midrule
    SI-SDR (dB) $\uparrow$ & $-1.60$ & $4.70$ & $+6.30$ \\
    L1 $\downarrow$ & $0.0312$ & $0.0176$ & $-0.0136$ \\
    MRSTFT $\downarrow$ & $1.945$ & $1.054$ & $-0.891$ \\
    MCD (dB) $\downarrow$ & $95.1$ & $36.2$ & $-59.0$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
Figure~\ref{fig:per_effect} shows SI-SDR improvement broken down by effect chain type. The model performs best on chains involving amplifier distortion, with amp + slapback + room reverb achieving +10.99\,dB and amp + reverb achieving +10.74\,dB. Pure amp distortion removal (NAM only) yields +7.20\,dB improvement.

Effects-only chains without amplifier distortion are harder to invert: chorus + reverb (+1.96\,dB), delay only (+1.65\,dB), and reverb only ($-0.14$\,dB). This is expected, as the model's training data is dominated by amp-based chains, and time-domain effects like reverb and delay are inherently more difficult to invert due to their long temporal dependencies.

For passthrough pairs (where the input is already clean), the model achieves an absolute SI-SDR of 11\,dB against the ground truth, indicating that it introduces minimal artifacts on unprocessed signals.
